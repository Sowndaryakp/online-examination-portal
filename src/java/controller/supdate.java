...
public class supdate extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (session.getAttribute("sname") != null && request.getParameter("submit") != null) {
                String sphone = request.getParameter("sphone");
                String semail = request.getParameter("semail");
                String ssem = request.getParameter("ssem");
                Registration u = new Registration(session);
                String status = u.supdate(sphone, semail, ssem);
                if (status.equals("success")) {
                    request.setAttribute("status", "Profile successfully Updated");
                    RequestDispatcher rd1 = request.getRequestDispatcher("sprofile.jsp");
